...
class Subhome3 extends CI_Controller {
    ...
    public function index($referrer_name, $campaign_name) {
        $_SESSION['referrer_name3'] = $referrer_name;
        $_SESSION['campaign_name3'] = $campaign_name;
        $this->load->view('subhome3_view', array('referrer_name' => $referrer_name, 'campaign_name' => $campaign_name));
    }

    public function grid($tipe = "", $start_date = "", $end_date = "") {
        $this->load->model('subgrid3_model', 'grid');
    ...
    public function chart($data_AFSiteID = "") {
        $this->load->model('subchart3_model', 'chart');
